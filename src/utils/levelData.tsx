/** @format */

//levelData.tsx

export const levelData1 = {
  prompt: "cats",
  gridSize: 9,
  singleImageMode: true,
  images: ["/images/1/1.png"],
  correctAnswers: [1, 2, 4, 5, 7, 8],
};

export const levelData2 = {
  prompt: "waldos",
  gridSize: 16,
  singleImageMode: true,
  images: ["/images/2/1.png"],
  correctAnswers: [4],
};

export const levelData3 = {
  prompt: "prime numbers",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/3/1.png",
      isCorrect: true,
    },
    {
      image: "/images/3/2.png",
      isCorrect: true,
    },
    {
      image: "/images/3/3.png",
      isCorrect: true,
    },
    {
      image: "/images/3/4.png",
      isCorrect: false,
    },
    {
      image: "/images/3/5.png",
      isCorrect: true,
    },
    {
      image: "/images/3/6.png",
      isCorrect: false,
    },
    {
      image: "/images/3/7.png",
      isCorrect: true,
    },
    {
      image: "/images/3/8.png",
      isCorrect: false,
    },
    {
      image: "/images/3/9.png",
      isCorrect: false,
    },
  ],
};

export const levelData4 = {
  prompt: "smooches",
  gridSize: 16,
  singleImageMode: true,
  images: ["/images/4/1.png"],
  correctAnswers: [7],
};

export const levelData17 = {
  prompt: "people who were alive in 1955",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/17/1.png",
      isCorrect: true,
    },
    {
      image: "/images/17/2.png",
      isCorrect: true,
    },
    {
      image: "/images/17/3.png",
      isCorrect: true,
    },
    {
      image: "/images/17/4.png",
      isCorrect: true,
    },
    {
      image: "/images/17/5.png",
      isCorrect: true,
    },
    {
      image: "/images/17/6.png",
      isCorrect: true,
    },
    {
      image: "/images/17/7.png",
      isCorrect: false,
    },
    {
      image: "/images/17/8.png",
      isCorrect: false,
    },
    {
      image: "/images/17/9.png",
      isCorrect: false,
    },
  ],
};

export const levelData6 = {
  prompt: "sandwiches",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/6/1.png",
      isCorrect: true,
    },
    {
      image: "/images/6/2.png",
      optional: true,
    },
    {
      image: "/images/6/3.png",
      optional: true,
    },
    {
      image: "/images/6/4.png",
      isCorrect: true,
    },
    {
      image: "/images/6/5.png",
      optional: true,
    },
    {
      image: "/images/6/6.png",
      optional: true,
    },
    {
      image: "/images/6/7.png",
      optional: true,
    },
    {
      image: "/images/6/8.png",
      optional: true,
    },
    {
      image: "/images/6/9.png",
      optional: true,
    },
  ],
};

export const levelData11 = {
  prompt: "chemical element abbreviations",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/11/1.png",
      isCorrect: true,
    },
    {
      image: "/images/11/2.png",
      isCorrect: false,
    },
    {
      image: "/images/11/3.png",
      isCorrect: false,
    },
    {
      image: "/images/11/4.png",
      isCorrect: false,
    },
    {
      image: "/images/11/5.png",
      isCorrect: true,
    },
    {
      image: "/images/11/6.png",
      isCorrect: false,
    },
    {
      image: "/images/11/7.png",
      isCorrect: false,
    },
    {
      image: "/images/11/8.png",
      isCorrect: true,
    },
    {
      image: "/images/11/9.png",
      isCorrect: true,
    },
  ],
};

export const levelData8 = {
  prompt: "loss",
  gridSize: 9,
  singleImageMode: true,
  images: ["/images/8/1.png"],
  correctAnswers: [1, 2, 3, 4, 5, 6, 7, 8, 9],
};

export const levelData19 = {
  prompt: "emojis available on iOS in 2020",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/19/1.png",
      isCorrect: true,
    },
    {
      image: "/images/19/2.png",
      isCorrect: false,
    },
    {
      image: "/images/19/3.png",
      isCorrect: false,
    },
    {
      image: "/images/19/4.png",
      isCorrect: true,
    },
    {
      image: "/images/19/5.png",
      isCorrect: true,
    },
    {
      image: "/images/19/6.png",
      isCorrect: true,
    },
    {
      image: "/images/19/7.png",
      isCorrect: true,
    },
    {
      image: "/images/19/8.png",
      isCorrect: false,
    },
    {
      image: "/images/19/9.png",
      isCorrect: true,
    },
  ],
};

export const levelData10 = {
  prompt: "gifs",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/10/1.png",
      isCorrect: false,
    },
    {
      image: "/images/10/2.png",
      isCorrect: false,
    },
    {
      image: "/images/10/3.png",
      isCorrect: false,
    },
    {
      image: "/images/10/1.gif",
      isCorrect: true,
    },
    {
      image: "/images/10/2.gif",
      isCorrect: true,
    },
    {
      image: "/images/10/3.gif",
      isCorrect: true,
    },
    {
      image: "/images/10/4.gif",
      isCorrect: true,
    },
    {
      image: "/images/10/5.gif",
      isCorrect: true,
    },
    {
      image: "/images/10/6.gif",
      isCorrect: true,
    },
  ],
};

export const levelData15 = {
  prompt: "things mentioned in the song We Didn't Start the Fire",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/15/1.png",
      isCorrect: false,
    },
    {
      image: "/images/15/2.png",
      isCorrect: false,
    },
    {
      image: "/images/15/3.png",
      isCorrect: false,
    },
    {
      image: "/images/15/4.png",
      isCorrect: true,
    },
    {
      image: "/images/15/5.png",
      isCorrect: false,
    },
    {
      image: "/images/15/6.png",
      isCorrect: false,
    },
    {
      image: "/images/15/7.png",
      isCorrect: true,
    },
    {
      image: "/images/15/8.png",
      isCorrect: true,
    },
    {
      image: "/images/15/9.png",
      isCorrect: false,
    },
  ],
};

export const levelData12 = {
  prompt: "Wordle tiles that should be yellow",
  gridSize: 25,
  singleImageMode: true,
  images: ["/images/12/1.png"],
  correctAnswers: [4, 5, 6, 7, 9, 10, 11, 12, 13, 15, 20],
};

export const levelData9 = {
  prompt: "U.S. states that share a name with a Sufjan Stevens album",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/9/1.png",
      isCorrect: true,
    },
    {
      image: "/images/9/2.png",
      isCorrect: false,
    },
    {
      image: "/images/9/3.png",
      isCorrect: false,
    },
    {
      image: "/images/9/4.png",
      isCorrect: false,
    },
    {
      image: "/images/9/5.png",
      isCorrect: false,
    },
    {
      image: "/images/9/6.png",
      isCorrect: false,
    },
    {
      image: "/images/9/7.png",
      isCorrect: true,
    },
    {
      image: "/images/9/8.png",
      isCorrect: false,
    },
    {
      image: "/images/9/9.png",
      isCorrect: false,
    },
  ],
};

export const levelData14 = {
  prompt:
    "a bear, a plane, a small silver jack, a yellow pencil, or a red thumbtack",
  gridSize: 16,
  singleImageMode: true,
  images: ["/images/14/1.png"],
  correctAnswers: [3, 5, 7, 16],
};

export const levelData7 = {
  prompt: "fonts displayed in their own typeface",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/7/1.png",
      isCorrect: true,
    },
    {
      image: "/images/7/2.png",
      isCorrect: true,
    },
    {
      image: "/images/7/3.png",
      isCorrect: false,
    },
    {
      image: "/images/7/4.png",
      isCorrect: true,
    },
    {
      image: "/images/7/5.png",
      isCorrect: true,
    },
    {
      image: "/images/7/6.png",
      isCorrect: true,
    },
    {
      image: "/images/7/7.png",
      isCorrect: false,
    },
    {
      image: "/images/7/8.png",
      isCorrect: true,
    },
    {
      image: "/images/7/9.png",
      isCorrect: false,
    },
  ],
};

export const levelData16 = {
  prompt: "toppings that go on pizza",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/16/1.png",
      optional: true,
    },
    {
      image: "/images/16/2.png",
      optional: true,
    },
    {
      image: "/images/16/3.png",
      optional: true,
    },
    {
      image: "/images/16/4.png",
      optional: true,
    },
    {
      image: "/images/16/5.png",
      optional: true,
    },
    {
      image: "/images/16/6.png",
      optional: true,
    },
    {
      image: "/images/16/7.png",
      optional: true,
    },
    {
      image: "/images/16/8.png",
      optional: true,
    },
    {
      image: "/images/16/9.png",
      optional: true,
    },
  ],
};

export const levelData5 = {
  prompt: "real company logos",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/5/1.png",
      isCorrect: true,
    },
    {
      image: "/images/5/2.png",
      isCorrect: false,
    },
    {
      image: "/images/5/3.png",
      isCorrect: false,
    },
    {
      image: "/images/5/4.png",
      isCorrect: false,
    },
    {
      image: "/images/5/5.png",
      isCorrect: false,
    },
    {
      image: "/images/5/6.png",
      isCorrect: true,
    },
    {
      image: "/images/5/7.png",
      isCorrect: true,
    },
    {
      image: "/images/5/8.png",
      isCorrect: false,
    },
    {
      image: "/images/5/9.png",
      isCorrect: true,
    },
  ],
};

export const levelData18 = {
  prompt: "sudoku boxes where the number 6 belongs",
  gridSize: 9,
  singleImageMode: true,
  images: ["/images/18/1.png"],
  correctAnswers: [1, 2, 3, 4, 5, 6, 7, 8, 9],
};

export const levelData13 = {
  prompt: "animals mentioned in a song by The Beatles",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/13/1.png",
      isCorrect: true,
    },
    {
      image: "/images/13/2.png",
      isCorrect: true,
    },
    {
      image: "/images/13/3.png",
      isCorrect: true,
    },
    {
      image: "/images/13/4.png",
      isCorrect: true,
    },
    {
      image: "/images/13/5.png",
      isCorrect: true,
    },
    {
      image: "/images/13/6.png",
      isCorrect: true,
    },
    {
      image: "/images/13/7.png",
      isCorrect: false,
    },
    {
      image: "/images/13/8.png",
      isCorrect: false,
    },
    {
      image: "/images/13/9.png",
      isCorrect: false,
    },
  ],
};

export const levelData20 = {
  prompt: "ingredients in the perfect egg bite",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/20/1.png",
      isCorrect: true,
    },
    {
      image: "/images/20/2.png",
      isCorrect: false,
    },
    {
      image: "/images/20/3.png",
      isCorrect: true,
    },
    {
      image: "/images/20/4.png",
      isCorrect: false,
    },
    {
      image: "/images/20/5.png",
      isCorrect: false,
    },
    {
      image: "/images/20/6.png",
      isCorrect: true,
    },
    {
      image: "/images/20/7.png",
      isCorrect: true,
    },
    {
      image: "/images/20/8.png",
      isCorrect: true,
    },
    {
      image: "/images/20/9.png",
      isCorrect: true,
    },
  ],
};

export const levelData21 = {
  prompt: "real national flags",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/21/1.png",
      isCorrect: false,
    },
    {
      image: "/images/21/2.png",
      isCorrect: false,
    },
    {
      image: "/images/21/3.png",
      isCorrect: false,
    },
    {
      image: "/images/21/4.png",
      isCorrect: true,
    },
    {
      image: "/images/21/5.png",
      isCorrect: true,
    },
    {
      image: "/images/21/6.png",
      isCorrect: true,
    },
    {
      image: "/images/21/7.png",
      isCorrect: false,
    },
    {
      image: "/images/21/8.png",
      isCorrect: false,
    },
    {
      image: "/images/21/9.png",
      isCorrect: false,
    },
  ],
};

export const levelData22 = {
  prompt: "an anchor, a musical note, a lock, a knight, or a billy goat",
  gridSize: 16,
  singleImageMode: true,
  images: ["/images/22/1.png"],
  correctAnswers: [2, 7, 12, 13, 16],
};

export const levelData23 = {
  prompt: "answers to previous prompts",
  gridSize: 9,
  singleImageMode: false,
  images: [
    {
      image: "/images/23/1.png",
      isCorrect: true,
    },
    {
      image: "/images/23/2.png",
      isCorrect: true,
    },
    {
      image: "/images/23/3.png",
      isCorrect: true,
    },
    {
      image: "/images/23/4.png",
      isCorrect: true,
    },
    {
      image: "/images/23/5.png",
      isCorrect: false,
    },
    {
      image: "/images/23/6.png",
      isCorrect: true,
    },
    {
      image: "/images/23/7.png",
      isCorrect: false,
    },
    {
      image: "/images/23/8.png",
      isCorrect: false,
    },
    {
      image: "/images/23/9.png",
      isCorrect: true,
    },
  ],
};

export const levelData24 = {
  prompt: "letters in a word that describes an inhabitant of our planet",
  gridSize: 16,
  singleImageMode: false,
  images: [
    {
      image: "/images/24/1.png",
      isCorrect: true,
    },
    {
      image: "/images/24/2.png",
      isCorrect: true,
    },
    {
      image: "/images/24/3.png",
      isCorrect: true,
    },
    {
      image: "/images/24/4.png",
      isCorrect: false,
    },
    {
      image: "/images/24/5.png",
      isCorrect: false,
    },
    {
      image: "/images/24/6.png",
      isCorrect: false,
    },
    {
      image: "/images/24/7.png",
      isCorrect: true,
    },
    {
      image: "/images/24/8.png",
      isCorrect: true,
    },
    {
      image: "/images/24/9.png",
      isCorrect: true,
    },
    {
      image: "/images/24/10.png",
      isCorrect: true,
    },
    {
      image: "/images/24/11.png",
      isCorrect: false,
    },
    {
      image: "/images/24/12.png",
      isCorrect: false,
    },
    {
      image: "/images/24/13.png",
      isCorrect: false,
    },
    {
      image: "/images/24/14.png",
      isCorrect: false,
    },
    {
      image: "/images/24/15.png",
      isCorrect: true,
    },
    {
      image: "/images/24/16.png",
      isCorrect: true,
    },
  ],
};

export const levelData25 = {
  prompt: "planets full of lovable beings who are doing their best",
  gridSize: 9,
  singleImageMode: true,
  images: ["/images/25/1.png"],
  correctAnswers: [1, 2, 3, 4, 5, 6, 7, 8, 9],
};
